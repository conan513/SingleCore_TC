...
void bot_ai::_getBotDispellableAuraList(Unit const* target, Unit const* caster, uint32 dispelMask, std::list<Aura const*> &dispelList) const
{
...
void bot_ai::_listAuras(Player const* player, Unit const* unit) const
{
    ...
    ch.PSendSysMessage("crit: %.2f pct", unit->GetUnitCriticalChanceDone(BASE_ATTACK));
    ch.PSendSysMessage("defense: %u", unit->GetDefenseSkillValue());
    ch.PSendSysMessage("miss : %.2f pct", unit->GetUnitMissChance());
    ...
    ch.PSendSysMessage("Damage mult mainhand: %.2f", unit->GetPctModifierValue(UNIT_MOD_DAMAGE_MAINHAND, BASE_PCT)*unit->GetPctModifierValue(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT));
    ch.PSendSysMessage("Attack time mainhand: %.2f (%.1f DPS)", attSpeed,
        ((unit->GetFloatValue(UNIT_FIELD_MINDAMAGE) + unit->GetFloatValue(UNIT_FIELD_MAXDAMAGE)) / 2) / attSpeed);
    if (unit->haveOffhandWeapon())
    {
        ...
        ch.PSendSysMessage("Damage mult offhand: %.2f", unit->GetPctModifierValue(UNIT_MOD_DAMAGE_OFFHAND, BASE_PCT)*unit->GetPctModifierValue(UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT));
        ch.PSendSysMessage("Attack time offhand: %.2f (%.1f DPS)", attSpeed,
            ((unit->GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE) + unit->GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE)) / 2) / attSpeed);
    }
    if (unit != me ||
        ...
        _botclass != BOT_CLASS_SHAMAN))
    {
        ...
        ch.PSendSysMessage("Damage mult ranged: %.2f", unit->GetPctModifierValue(UNIT_MOD_DAMAGE_RANGED, BASE_PCT)*unit->GetPctModifierValue(UNIT_MOD_DAMAGE_RANGED, TOTAL_PCT));
        ch.PSendSysMessage("Attack time ranged: %.2f (%.1f (%.1f from ammo) DPS)", attSpeed,
            ((unit->GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE) + unit->GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE)) / 2) / attSpeed,
    ...
    if (unit == me)
    {
        ...
        {
            ch.PSendSysMessage("mana regen_5 casting: %.2f", (_botclass == BOT_CLASS_SPHYNX ? -1.f : 1.f) * me->GetFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER) * sWorld->getRate(RATE_POWER_MANA) * 5.0f);
            ch.PSendSysMessage("mana regen_5 no cast: %.2f", (_botclass == BOT_CLASS_SPHYNX ? -1.f : 1.f) * me->GetFloatValue(UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER) * sWorld->getRate(RATE_POWER_MANA) * 5.0f);
        }
        ...
        if (!IAmFree())
            ch.PSendSysMessage("Follow distance: %u", master->GetBotMgr()->GetBotFollowDist());
...
{
    //TC_LOG_ERROR("entities.unit", "CheckAuras: %u force %u temp %u lastdiff %u",
    //    me->GetEntry(), uint32(force), uint32(IsTempBot()), lastdiff);
    if (!force && !IsTempBot())
    {
        Regenerate();
        _updateRations(); //safe
    ...
    if (checkAurasTimer <= lastdiff)
    {
            ...
            !me->isMoving() && !IsCasting() && !IsShootingWand())
        {
            ...
            else
            {
                ...
                if (me->GetExactDist2d(&attackpos) > 4.f || !me->IsWithinLOSInMap(opponent))
...
void bot_ai::Regenerate()
{
    ...
    if (regenTimer >= REGEN_CD)
    {
        regenTimer -= REGEN_CD;
        // Regen Health
        if ((!me->IsInCombat() || me->IsPolymorphed() || CanRegenInCombat()) && me->GetHealth() < me->GetMaxHealth())
        {
...
void bot_ai::ApplyItemBonuses(uint8 slot)
{
    ...
    if (item->GetEntry() != einfo->ItemEntry[slot])
    {
        ...
        {
            _stats[slot][BOT_STAT_MOD_DAMAGE_MIN] += proto->Damage[0].DamageMin + proto->Damage[1].DamageMin + proto->Damage[2].DamageMin;
            _stats[slot][BOT_STAT_MOD_DAMAGE_MAX] += proto->Damage[0].DamageMax + proto->Damage[1].DamageMax + proto->Damage[2].DamageMax;
        }
...
inline bool bot_ai::CCed(Unit const* target, bool root)
{
