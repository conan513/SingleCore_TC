...
void BotMgr::LoadConfig(bool reload)
{
    ...
    //limits
    _mult_dmg_melee         = std::max<float>(_mult_dmg_melee, 0.1f);
    _mult_dmg_spell         = std::max<float>(_mult_dmg_spell, 0.1f);
    _mult_healing           = std::max<float>(_mult_healing,   0.1f);
    _mult_dmg_melee         = std::min<float>(_mult_dmg_melee, 10.f);
...
{
    return _dpstracker->GetDPSTaken(u->GetGUID().GetCounter());
}
