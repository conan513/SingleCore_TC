...
void DPSTracker::_Release()
{
    for (DamageTakenMap::const_iterator itr = _damages.begin(); itr != _damages.end(); ++itr)
    {
        ...
        //shift
        for (uint8 i = MAX_DAMAGES-1; i > 0; --i)
            dmgs[i] = dmgs[i-1];
        dmgs[0] = 0;
    }
}

void DPSTracker::_AccumulateDamage(uint32 guidlow, uint32 damage)
{
    DamageTakenMap::const_iterator itr = _damages.find(guidlow);
...
void DPSTracker::TrackDamage(Unit const* victim, uint32 damage)
{
    ...
    _SetActive();
    _AccumulateDamage(victim->GetGUID().GetCounter(), damage);
}
...
uint32 DPSTracker::GetDPSTaken(uint32 guidlow) const
{
    DPSTakenMap::const_iterator itr = _DPSes.find(guidlow);
    //TC_LOG_ERROR("entities.player", "DPSTracker::GetDPSTaken(): from %u, damage %u", guidlow, itr != _DPSes.end() ? itr->second : 0);
